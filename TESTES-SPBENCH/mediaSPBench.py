...
def calcular_media_pandas(prefixo):
    ...
    for filename in os.listdir(directory):
        if filename.startswith(prefixo):
            ...
            for thread in [1, 2, 3]:
                latency_values = df[df['Thread'] == thread]['Average_latency']
